...
public interface CandidateRepo extends CrudRepository<Candidate, Long> {
    ...
    @Query("select c from Candidate c where c.party = :party order by c.id asc")
    ...
    @Query("select c from Candidate c where c.residence like :place and c.party = :party order by c.id asc")
    List<Candidate> findAllByResidenceContainsAndPartyByOrderByIdAsc(@Param("place") String place,@Param("party") String party);

    @Query("select c from Candidate c where c.fullname like :name and c.party = :party order by c.id asc")
    ...
    @Query("select c from Candidate c where c.fullname like :name and c.residence like :place and c.party = :party order by c.id asc")
